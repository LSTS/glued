...
target_install()
{
    ...
    $cmd_make CROSS_COMPILE="$cfg_target_canonical"- CONFIG_PREFIX=$cfg_dir_output_rootfs install &&
    tar -C "$pkg_dir/fs" --exclude .svn -c -f - . | tar -C "$cfg_dir_output_rootfs" -x -v -f - &&
    tar -C ".tmpftp" -c -f - . | tar -C "$cfg_dir_output_rootfs" -x -v -f -
    
    rm -Rf .tmpftp

    tar -czf ../busybox-v$version.tar.gz ../rootfs
}
